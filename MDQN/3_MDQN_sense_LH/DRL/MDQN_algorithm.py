...
def train_test_MDQN(**kwargs):
    ...
    use_delayed_cost_assignment = kwargs.get("use_delayed_cost_assignment", True)
    include_weights_in_log = kwargs.get("include_weights_in_log", True)
    reward_delay_time = env.reward_delay_time
    if not use_delayed_cost_assignment:
        reward_delay_time = 0
    ...
    log = {
        ...
        "step": best_step,
        "selected_head": agent.current_head + 1
    }
    if include_weights_in_log:
        log["parameters"] = agent.get_parameters()
    else:
        log["parameters"] = {"head": agent.current_head}
    return log
