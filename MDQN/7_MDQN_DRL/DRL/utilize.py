...
class ReplayBufferWithDone(ReplayBuffer):
    def clear(self):
        self.size = 0
        [self.obs_buf,
         self.action_buf,
         self.reward_buf,
         self.obs_next_buf,
         self.done_buf] = [deque(maxlen=self.max_size) for _ in range(5)]

    def store(self, transition: Tuple[np.ndarray, float, float, np.ndarray, float]):
        transition = copy(transition)
        self.obs_buf.append(transition[0])
        self.action_buf.append(transition[1])
        self.reward_buf.append(transition[2])
        self.obs_next_buf.append(transition[3])
        self.done_buf.append(float(transition[4]))
        self.size += 1
        self.size = min(self.size, self.max_size)
...
class RolloutBuffer:
    ...
    def store(self,
              ...
              log_prob:float,
              value:float):
        self.obs_buf.append(copy(state))
    ...
    def get(self, gamma:float, gae_lambda:float, last_value:float):
        ...
        next_values = np.concatenate((values[1:], np.array([last_value], dtype=np.float32)))
        ...
        return (
            np.stack(self.obs_buf, axis=0),
            np.array(self.action_buf, dtype=np.int32),
            np.array(self.log_prob_buf, dtype=np.float32),
