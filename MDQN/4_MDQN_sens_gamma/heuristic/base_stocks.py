...
class BaseStock(Agent):
    ...
    def train(self, length:int=None, repeats:int=None, gamma:float=1.0, seeds=None, n_jobs:int=-1):
        parameters_list = [{"base_stock": base_stock} for base_stock in range(self.max_inventory + 1)]
        log = evaluate_policy_multi_process(
            self,
            self.env,
            length,
            repeats,
...
class CappedBaseStock(Agent):
    ...
    def train(self, length:int, repeats:int, gamma:float=1.0, seeds=None, n_jobs:int=-1):
