...
def train_test_MDQN(**kwargs):
    ...
    gamma_eval = kwargs.get("gamma_eval", gamma)
    # 2.Initialize
    # 2.1 environments
    ...
    for train_step in range(train_steps + 1):
        ...
        if train_step % eval_frq == 0:
            ...
            for idx_head in candidate_heads:
                eval_log, replay_buffer_new = _evalMDQN(eval_env,
                                                        agent,
                                                        cost_cache,
                                                        replay_buffer,
                                                        n_repeats_eval,
                                                        gamma_eval,
                                                        idx_head=idx_head)
                performances_eval[idx_head] = eval_log["performance"]
                mean_costs_eval[idx_head] = eval_log["mean_cost"]
                new_replay_buffers[idx_head] = replay_buffer_new
                scores[idx_head] = _update_ema_score(scores[idx_head], eval_log["performance"], w)
            ...
            mean_cost_log = mean_costs_eval[current_head]
            performance_history.append(performance_log)
            mean_cost_history.append(mean_cost_log)
    ...
    test_log = _testMDQN(test_env, agent, test_repeats, gamma_eval)
    performance_test = test_log["performance"]
    if is_print:
        print(
    ...
    log = {
        ...
        "history_mean_cost": mean_cost_history,
        "performance": performance_test,
        "mean_cost": test_log["mean_cost"],
        "mean_assigned_cost": test_log["mean_assigned_cost"],
        "raw_discounted_cost": test_log["raw_discounted_cost"],
        "step": best_step,
        "parameters": agent.get_parameters(),
        "metric": "normalized_discounted_cost",
        "gamma_eval": gamma_eval,
    }
    return log
            

def _testMDQN(env:gym.Env, agent:MDQN, n_repeats:int=10, gamma_eval:float=1.0, idx_head:int=None):
    episode_costs = []
    env = copy(env)
...
def _evalMDQN(env:gym.Env, agent:MDQN, cost_cache:CostDelayCache, replay_buffer:ReplayBuffer, n_repeats:int=10, gamma_eval:float=1.0, idx_head:int=None):
