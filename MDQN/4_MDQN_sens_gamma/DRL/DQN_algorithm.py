...
def train_test_DQN(**kwargs):
    ...
    for train_step in range(train_steps + 1):
        ...
        if train_step % eval_frq == 0:
            eval_log = testDQN(eval_env, agent, n_repeats_eval, gamma_eval, is_eval=True)
            performance_eval = eval_log["performance"]
            performance_history_eval.append(performance_eval)
            mean_cost_history_eval.append(eval_log["mean_cost"])
    ...
    test_log = testDQN(test_env, agent, test_repeats, gamma_eval)
    ...
    log = {
        ...
        "history_mean_cost": mean_cost_history_eval,
...
def testDQN(env:gym.Env, agent:DQN, n_repeats:int=10, gamma_eval:float=1.0, is_eval:bool=False):
    episode_costs = []
    if not is_eval:
        env = copy(env)
