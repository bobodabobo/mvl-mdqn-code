...
XIN2020_LEAD_TIMES = (1, 2, 3, 4)
XIN2020_PENALTY_COSTS = (4, 9, 19, 39)
XIN2020_DEMAND_DISTS = ("poisson", "geometric")
...
def build_xin2020_lost_sale_configs():
    configs = []
    instance_idx = 1
    for demand_dist in XIN2020_DEMAND_DISTS:
        for penalty_cost in XIN2020_PENALTY_COSTS:
            for lead_time in XIN2020_LEAD_TIMES:
                instance_id = f"XLS{instance_idx:02d}"
                configs.append(
                    {
                        ...
                        "lead_time": lead_time,
                        "ordering_cost_rate": 0.0,
                        "shortage_cost_rate": float(penalty_cost),
                        "holding_cost_rate": 1.0,
                        "penalty_cost": float(penalty_cost),
                        "max_order_quantity": 15,
                        "max_inventory": 45,
                        "inventory_clip": None,
                        "heuristic_base_stock_max": 45,
                        "heuristic_cap_max": 15,
                        "max_steps": 1000,
                    }
                )
                instance_idx += 1
...
xin2020_lost_sale_configs = build_xin2020_lost_sale_configs()


def get_xin2020_lost_sale_configs():
    return [copy(config) for config in xin2020_lost_sale_configs]
...
class DemandGenerator:
    ...
    def __init__(self, mean:int, seed:int=None, demand_dist:str="poisson"):
        if demand_dist == "poisson":
            ...
            raise ValueError(f"Invalid demand distribution {demand_dist}.")
...
class LostSalesInventory(gym.Env):
    ...
    def __init__(self, config:dict):
        super().__init__()
        # parameters
        self.instance_id = config.get("instance_id", None)
        self.task_name = config.get("task_name", self.instance_id)
        ...
        self.penalty_cost = config.get("penalty_cost", self.shortage_cost_rate)
        self.max_order_quantity = config["max_order_quantity"]
        self.max_inventory = config["max_inventory"]
        self.inventory_clip = config.get("inventory_clip", self.max_inventory)
        self.max_steps = config["max_steps"]
        self.reward_delay_time = self.lead_time
        self.heuristic_base_stock_max = config.get("heuristic_base_stock_max", None)
        self.heuristic_cap_max = config.get("heuristic_cap_max", None)
        # components
    ...
    def step(self, action):
        ...
        if self.inventory_clip is not None and self.inventory_on_hand > self.inventory_clip:
            truncated = True
        else:
            truncated = False
        if self.inventory_clip is not None:
            self.inventory_on_hand = min(self.inventory_on_hand, self.inventory_clip)
        # sell
