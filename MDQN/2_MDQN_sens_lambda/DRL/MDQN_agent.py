...
class MDQN(Agent):
    ...
    def _build_normalized_loss_weights(n_heads:int, lambda_anc:float):
        ...
        weights = weights + lambda_anc * sap_weights
        # Normalize by the mean so lambda_anc only changes the relative
        # emphasis across horizons without inflating the total loss scale.
