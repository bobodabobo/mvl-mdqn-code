...
class MDQN(Agent):
    ...
    def __init__(self,
        ...
        self.net = MultiHeadDuelingDQN(self.state_size, self.action_size, n_heads).to(self.device)
        self.target_net = copy(self.net).to(self.device)
        self.current_head = 0
        self.opt = torch.optim.AdamW(self.net.parameters(), lr=lr)
        self.rng = np.random.default_rng(seed)
        self.horizons = torch.arange(1, self.n_heads + 1, dtype=torch.float32, device=self.device).view(1, -1)
        self.prev_ratios = (self.horizons - 1.0) / self.horizons
        self.loss_weights = self._build_normalized_loss_weights(self.n_heads, self.lambda_anc).to(self.device)
