...
def train_test_DQN(**kwargs):
    ...
    device = kwargs.get("device", DQN_config["device"])
    ...
    if DRL_method in ['DQN', "DQN-L"]:
        agent = DQN(train_env.observation_space.shape[0], train_env.action_space.n, lr, seed, device)
    elif DRL_method in ['TDQN', 'TDQN-L']:
        agent = TDQN(train_env.observation_space.shape[0], train_env.action_space.n, lr, seed, device)
    elif DRL_method in ['DDQN', 'DDQN-L']:
        agent = DDQN(train_env.observation_space.shape[0], train_env.action_space.n, lr, seed, device)
    ...
    for train_step in range(train_steps + 1):
        ...
        if DRL_method in ['RSDQN', 'RSDQN-L']:
            teacher_state = train_env.extract_inventory_obs(state)
            action_teacher = teacher_agent.act(teacher_state)
