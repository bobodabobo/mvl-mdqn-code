...
def _evaluate_policy_single_process(agent:Agent, env:Env, length:int=None, repeats:int=1):
    ...
    for idx_repeat in range(repeats):
        ...
        while not terminated:
            inventory_obs = env.extract_inventory_obs(obs) if hasattr(env, "extract_inventory_obs") else obs
            action = agent.act(inventory_obs)
...
def evaluate_policy_multi_process(agent, env, length, repeats, parameters_list):
    time_start = time()
    tasks = []
    for parameters in parameters_list:
        agent = agent.__class__(env, parameters)
        tasks.append([agent, env, length, repeats])
    performances = Parallel(n_jobs=-1, 
                            backend="loky",
