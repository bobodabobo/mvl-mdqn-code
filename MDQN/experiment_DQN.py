...
def DQN_experiment():
    ...
    for task_idx in range(N_tasks):
        ...
        for DRL_method in DRL_methods:
            print(DRL_method)
            t00 = time()
            dqn_params = {'DRL_method': DRL_method,
                          'env': env,
                          'task_name': task_name,
            ...
            if DRL_method in RSDQN_methods:
                ...
                results = run_parallel_seeds(train_test_DQN, RL_seeds, teacher_agent=teacher_agent, **dqn_params)
            else:
                results = run_parallel_seeds(train_test_DQN, RL_seeds, **dqn_params)
    ...
    for task_idx in range(N_tasks):
        ...
        for DRL_method in DRL_methods:
            t00 = time()
            print(DRL_method)
            dqn_params = {'DRL_method': DRL_method, 'env': env, 'task_name': task_name, 'test_repeats': TEST_REPEATS}
            if DRL_method in RSDQN_methods:
